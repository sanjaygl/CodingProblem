...
CLR is the execution engine of .NET that runs managed code.
It handles memory management, garbage collection, security, and exception handling.

## IL / MSIL (Intermediate Language)
IL is a platform-independent intermediate code generated by the .NET compiler.
It is converted into machine code at runtime by the JIT compiler.

## JIT (Just-In-Time Compiler)
JIT converts IL code into native machine code at runtime.
It compiles methods only when they are called for the first time.

## CTS (Common Type System)
CTS defines how data types are declared, used, and managed in .NET.
It ensures type safety and allows different .NET languages to share types.

## CLS (Common Language Specification)
CLS is a set of rules that .NET languages must follow to work together.
It ensures language interoperability across different .NET languages.

## Garbage Collector (GC)
GC automatically manages memory by removing unused objects.
It helps prevent memory leaks and improves application stability.

## Metadata
Metadata contains information about types, methods, and assemblies.
CLR uses metadata for type checking, security, and execution.

## Managed Code
Managed code is code executed under the control of the CLR.
It benefits from services like GC, security, and exception handling.

## Unmanaged Code
Unmanaged code runs directly on the operating system.
It does not use CLR services like garbage collection.

## Assembly
An assembly is a compiled .NET unit (.dll or .exe).
It contains IL code, metadata, and manifest information.

for more details please look **[CLR (Common Language Runtime) - Complete Guide](docs/csharp/CLR-Complete-Guide.md)**
